...
int sumFitness, minFitness, maxFitness, avgFitness;

// 计算适应度时，用的惩罚函数系数
//double alpha;

// 一个种群的最大和最小适应度个体
int minPopLocaion, maxPopLocation;
...
void report(struct population* pop, int gen) {
	...
	for (int j = 0; j < CHROM_SIZE; j++) {
		if (j % 5 == 0) { printf(" "); }
		// 每个基因单位输出
		printf("%1d", pop[maxPopLocation].chrom[j]);
	}
	printf("\nThe population's max fitness is %d.", (int)pop[maxPopLocation].fitness);
	printf("\nThe population's max weight is %d.\n", (int)pop[maxPopLocation].weight);
}
...
void main(int argc, char* argv[]) {
	...
	//statistics( newPop );            // 获取新种群的信息（适应度等）
	statistics(oldPop);                 //获取第一代种群的信息
	// report( newPop, gen );
	report(oldPop, gen);
	while (gen < GENERATION_NUM) {
		...
		oldMax = maxFitness;        // 记录上一代最大适应度
		oldMaxPopLocation = maxPopLocation;            // 记录上一代最大适应度个体位置
		generation();
		statistics(newPop);
		//report(newPop, gen);

		// 如果新的最大适应度个体的适应度小于上一代最大个体的适应度，则上一代活着
		if (maxFitness < oldMax) {
			for (k = 0; k < CHROM_SIZE; k++) {
				newPop[maxPopLocation].chrom[k] = oldPop[oldMaxPopLocation].chrom[k];
			}
			newPop[maxPopLocation].fitness = oldPop[oldMaxPopLocation].fitness;
			newPop[maxPopLocation].parent1 = oldPop[oldMaxPopLocation].parent1;
			newPop[maxPopLocation].parent2 = oldPop[oldMaxPopLocation].parent2;
			newPop[maxPopLocation].cross = newPop[minPopLocaion].cross;
			statistics(newPop); //更新种群（保留了上代最大适应度个体）
			report(newPop, gen);
		}
