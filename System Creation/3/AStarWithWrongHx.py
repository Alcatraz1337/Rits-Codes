...
def someUnknownDist(a, b):
    dist = manhatannDist(a, b) * 10
    return dist

def AStarSearchIncorrect(maze_map, start, goal, expCost):
    # Initialize files to save the estCost-Openlist and CloesdList
    # fcost (cost already taken + Est.cost) for moving to different location is stored in totalCost-OpenList-AStarSearchIncorrect.txt
    # With format:(Tot.cost, (x, y))
    folTxt = open("fCost-OpenList-AStarSearchIncorrect.txt", mode= 'w+')
    clTxt = open("ClosedList-AStarSearchIncorrect.txt", mode= 'w+')
    ...
    h_cost = [someUnknownDist(np.array(start), np.array(goal))]
    ...
    while openList:
        ...
        if s == goal:
            print("Finished!")
            plt.title("Search Result - Using A* Search - Wrong H(x)")
        ...
        for p in currentPos + direction:
            ...
            if tuple(p) not in openList:
                ...
                h_cost.append(someUnknownDist(p, np.array(goal)))
                # f_cost.append(temp_g_cost + manhatannDist(p, np.array(goal)))
                f_cost.append(temp_g_cost + someUnknownDist(p, np.array(goal)))
            ...
            if isBetter: 
                ...
                g_cost.remove(g_cost[idx])
                g_cost.insert(idx, temp_g_cost)
                h_cost.remove(h_cost[idx])
                h_cost.insert(idx, someUnknownDist(p, np.array(goal)))
                # h_cost.insert(idx, manhatannDist(p, np.array(goal)))
                f_cost.remove(f_cost[idx])
                # f_cost.insert(idx, temp_g_cost + manhatannDist(p, np.array(goal)))
                f_cost.insert(idx, temp_g_cost + someUnknownDist(p, np.array(goal)))
...
if __name__ == "__main__":
    ...
    AStarSearchIncorrect(maze, (1, 1), (height - 2, width - 2), cost)
